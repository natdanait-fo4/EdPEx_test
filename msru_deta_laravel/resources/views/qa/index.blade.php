...
@section('title', 'คำถามที่พบบ่อย (FAQ) และถามตอบ (QA) | NSRU-MS DETA SHIP')

@section('skeleton_content')
    <!-- QA Hero Skeleton -->
    <div style="height: 250px; border-radius: 16px; margin-bottom: 24px;" class="skeleton-shimmer"></div>

    <!-- QA Layout Skeleton -->
    <div style="display: flex; gap: 30px; flex-wrap: wrap;">
        <!-- Left Column: FAQ List Skeleton -->
        <div style="flex: 1; min-width: 300px; display: flex; flex-direction: column; gap: 16px;">
            <div style="height: 30px; width: 200px; border-radius: 4px; margin-bottom: 8px;" class="skeleton-shimmer"></div>
            <div style="height: 60px; border-radius: 8px;" class="skeleton-shimmer"></div>
            <div style="height: 60px; border-radius: 8px;" class="skeleton-shimmer"></div>
            <div style="height: 60px; border-radius: 8px;" class="skeleton-shimmer"></div>
            <div style="height: 60px; border-radius: 8px;" class="skeleton-shimmer"></div>
        </div>
        ...
        <div style="width: 300px; flex-shrink: 0; display: flex; flex-direction: column; gap: 24px;">
            <!-- Ask Card -->
            <div style="height: 180px; border-radius: 12px;" class="skeleton-shimmer"></div>
            <!-- History Card -->
            <div style="height: 300px; border-radius: 12px;" class="skeleton-shimmer"></div>
        </div>
    </div>
@endsection
...
@section('content')
    ...
    <div class="qa-hero">
        <div class="container">
            <h2>คำถามที่พบบ่อย (FAQ) และถามตอบ (QA)</h2>
    ...
    <div class="qa-main-wrapper">
        <div class="container qa-layout">
                        ...
                        @foreach($faqs as $index => $faq)
                            <div class="faq-item {{ $index === 0 ? 'active' : '' }}" data-category="{{ $faq->category }}">
                                <div class="faq-question">
                                    <span class="faq-q-text"><span class="faq-icon-text">{{ $index === 0 ? '[-]' : '[+]' }}</span>
                                        {{ $faq->question }}</span>
                            ...
                            @foreach($user_questions as $q)
                                <div class="faq-item">
                                    ...
                                    <div class="faq-answer" style="display: none;">
                                        ...
                                        <div
                                            style="background: #fdf2ff; border-left: 3px solid var(--primary-color); padding: 15px; border-radius: 4px; border: 1px solid #f5e6ff; background-color: #fdf8ff;">
                                            <div
                                                style="font-size: 0.75rem; font-weight: 700; color: var(--primary-color); text-transform: uppercase; margin-bottom: 5px;">
                                                คำตอบจากทางคณะ:</div>
                                        ...
                                        <div style="margin-top: 10px; font-size: 0.75rem; color: #999; text-align: right;">
                                            <i class="fa-regular fa-clock"></i> เมื่อวันที่
            ...
            <div class="qa-right-col">
                <div class="qa-right-spacer"></div>
                <div class="ask-card">
                    <h3>ไม่พบคำตอบที่ต้องการ?</h3>
                    <p>คุณสามารถตั้งคำถามใหม่เพื่อให้เจ้าหน้าที่ของเราช่วยตอบได้</p>
                    <button class="btn-ask"><i class="fa-solid fa-plus"></i> ตั้งคำถาม</button>
                </div>
                    ...
                    @if(auth()->check() && $my_questions->count() > 0)
                        <div class="history-list" style="margin-top: 15px;">
                            @foreach($my_questions as $q)
                                <div class="mb-3 rounded-lg history-item-click" data-title="{{ $q->title }}"
                                    data-details="{{ $q->details }}" data-answer="{{ $q->answer }}" data-status="{{ $q->status }}"
                                    ...
                                    <div
                                        style="font-size: 0.8rem; color: #888; display: flex; justify-content: space-between; align-items: center;">
                                        <span>
                                            <i class="fa-solid fa-circle text-[8px] mr-1"
                                                style="color: {{ $q->status == 'answered' ? '#10b981' : '#f59e0b' }}"></i>
                                            {{ $q->status == 'answered' ? 'ตอบแล้ว' : 'รอการตอบกลับ' }}
                                        </span>
                                        <div style="display: flex; gap: 12px; align-items: center;">
                                            <span>{{ \Carbon\Carbon::parse($q->created_at)->diffForHumans() }}</span>
                                            @if($q->status == 'waiting')
                                                <form action="{{ route('qa.user_destroy', $q->id) }}" method="POST"
                                                    onsubmit="event.stopPropagation(); return confirm('คุณต้องการลบคำถามนี้ใช่หรือไม่?');"
                                                    style="margin: 0; padding: 0;">
                                                    @csrf
                                                    <button type="submit"
                                                        style="background: none; border: none; color: #ef4444; font-size: 0.85rem; cursor: pointer; padding: 0; opacity: 0.8; transition: 0.2s;"
            ...
            <form action="{{ route('qa.store') }}" method="POST" id="qaForm">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="qaTitle" class="form-control"
                            placeholder="เช่น สอบถามเรื่องค่าเทอมสาขาคอมธุรกิจ">
                        <div id="qaTitleError" class="error-text"><i class="fa-solid fa-asterisk"></i> กรุณากรอกหัวข้อเรื่อง
                        </div>
                    </div>

                    <div class="form-group">
                        <label>รายละเอียดคำถาม</label>
                        <textarea name="details" id="qaDesc" class="form-control" rows="4"
                            placeholder="อธิบายรายละเอียดหรือข้อสงสัยของคุณเพิ่มเติม..."></textarea>
                        <div id="qaDescError" class="error-text"><i class="fa-solid fa-asterisk"></i>
                            กรุณากรอกรายละเอียดคำถาม</div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <p class="privacy-note" id="privacyNote">* คำถามแบบสาธารณะจะแสดงในหน้า FAQ
                            เพื่อเป็นประโยชน์ต่อผู้อื่น</p>
                    </div>
    ...
    <div id="viewReplyModal" class="modal-overlay">
        <div class="modal-content" style="max-width: 600px;">
            <div class="modal-header">
                ...
                <span class="close-view-modal"
                    style="cursor:pointer; font-size:1.5rem; font-weight:bold; color:#888;">&times;</span>
            </div>
            <div class="modal-body">
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="font-size: 0.8rem; color: #999; margin-top: 5px;"><i class="fa-regular fa-clock"></i>
                        ถามเมื่อ: <span id="viewReplyDate"></span></div>
                </div>

                <div style="margin-bottom: 20px;">
                    <div style="font-size: 0.85rem; color: #666; margin-bottom: 5px;">รายละเอียดคำถาม</div>
                    <div id="viewReplyDetails"
                        style="background: #f9f9f9; padding: 12px; border-radius: 6px; font-size: 0.95rem; color: #444; white-space: pre-wrap; font-family: inherit; line-height: 1.5;">
                    </div>
                </div>

                <div id="viewReplyAnswerSection" style="display: none; margin-top:20px;">
                    <div style="font-size: 0.85rem; color: var(--primary-color); font-weight: 600; margin-bottom: 5px;">
                        คำตอบจากทางคณะ</div>
                    <div id="viewReplyAnswer"
                        style="background: #fdf8ff; border-left: 3px solid var(--primary-color); padding: 15px; border-radius: 4px; border: 1px solid #f5e6ff; font-size: 0.95rem; color: #333; white-space: pre-wrap; font-family: inherit; line-height: 1.5;">
                    </div>
                </div>

                <div id="viewReplyWaitingSection"
                    style="display: none; margin-top:20px; text-align: center; padding: 20px; background: #fffbeb; border: 1px solid #fef3c7; border-radius: 6px; color: #b45309;">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            qaTags.forEach(tag => {
                tag.addEventListener('click', function () {
                    ...
                    // ล้าง search เมื่อกด tag
                    const searchInput = document.querySelector('.qa-search-input');
                    if (searchInput) searchInput.value = '';
                    const noRes = document.getElementById('qa-no-result');
                    ...
                    faqItems.forEach(item => {
                        ...
                        if (item.querySelector('.faq-answer')) item.querySelector('.faq-answer').style.display = 'none';
                        if (item.querySelector('.faq-question i')) item.querySelector('.faq-question i').className = 'fa-solid fa-chevron-down';
                        if (item.querySelector('.faq-icon-text')) item.querySelector('.faq-icon-text').textContent = '[+]';
                    });
                    const publicSection = document.querySelector('.public-inquiries-section');
            ...
            // สร้างกล่องข้อความ "ไม่พบผลลัพธ์"
            const noResultMsg = document.createElement('div');
            noResultMsg.id = 'qa-no-result';
            noResultMsg.style.cssText = 'display:none; text-align:center; padding:40px 20px; color:#9ca3af;';
            noResultMsg.innerHTML = '<i class="fa-solid fa-magnifying-glass fa-2x" style="margin-bottom:12px; display:block; color:#d1d5db;"></i><div style="font-size:1rem; font-weight:600; margin-bottom:6px; color:#6b7280;">ไม่พบผลลัพธ์</div><div style="font-size:0.875rem;">ลองใช้คำค้นหาอื่น หรือตั้งคำถามใหม่</div>';
            const faqListEl = document.querySelector('.faq-list');
            if (faqListEl) faqListEl.parentNode.insertBefore(noResultMsg, faqListEl.nextSibling);

            if (searchInput) {
                searchInput.addEventListener('input', function () {
                    const query = this.value.trim().toLowerCase();

                    // รีเซ็ต tag filter เป็น "ทั้งหมด"
                    qaTags.forEach(t => t.classList.remove('active'));
                    const allTag = document.querySelector('.qa-tag[data-target="all"]');
                    ...
                    // กรอง FAQ Items หลัก
                    faqItems.forEach(item => {
                        const questionText = (item.querySelector('.faq-q-text') ? item.querySelector('.faq-q-text').textContent : '').toLowerCase();
                        const answerText   = (item.querySelector('.faq-answer')  ? item.querySelector('.faq-answer').textContent  : '').toLowerCase();
                        const match = questionText.includes(query) || answerText.includes(query);
                        item.style.display = match ? 'block' : 'none';
                        if (match) {
                            hasResult = true;
                            item.classList.add('active');
                            if (item.querySelector('.faq-answer'))    item.querySelector('.faq-answer').style.display = 'block';
                            if (item.querySelector('.faq-question i')) item.querySelector('.faq-question i').className = 'fa-solid fa-chevron-up';
                            if (item.querySelector('.faq-icon-text'))  item.querySelector('.faq-icon-text').textContent = '[-]';
                        } else {
                            item.classList.remove('active');
                            if (item.querySelector('.faq-answer'))    item.querySelector('.faq-answer').style.display = 'none';
                    ...
                    // กรอง User Questions section
                    const publicSection = document.querySelector('.public-inquiries-section');
                    if (publicSection) {
                        const userItems = publicSection.querySelectorAll('.faq-item');
                        ...
                        publicSection.style.display = userHasResult ? 'block' : 'none';
                    }
